...
class FileIO : public QObject {
   ...
   public:
      ...
      /** Reads file contents as a binary string; each character code is one byte. */
      Q_INVOKABLE QString readBinary();
      /** Reads file contents and returns Base64-encoded binary data. */
      Q_INVOKABLE QString readBinaryBase64();
      ...
      /** Writes Base64-encoded binary data to the file. */
      Q_INVOKABLE bool writeBinaryBase64(const QString& data);
      /** Writes hexadecimal binary data to the file. Whitespace is ignored. */
      Q_INVOKABLE bool writeBinaryHex(const QString& data);
      /** Writes an array of byte values to the file. */
      Q_INVOKABLE bool writeBytes(const QVariantList& data);
      /** Removes the file */
...
class MsProcess : public QProcess {
      ...
      Q_PROPERTY(int exitStatus READ exitStatusValue)
      Q_PROPERTY(int error READ processError)
      Q_PROPERTY(QString errorString READ errorString)

   public:
      MsProcess(QObject* parent = 0) : QProcess(parent) {}
      int exitStatusValue() const { return int(QProcess::exitStatus()); }
      int processError() const    { return int(QProcess::error());      }

   public slots:
      //@ --
      Q_INVOKABLE void start(const QString& program)      { QProcess::start(program); }
      //@ --
      Q_INVOKABLE void startWithArgs(const QString& program, const QStringList& args) { QProcess::start(program, args, QIODevice::ReadWrite); }
      //@ --
