...
bool MuseScore::importExtension(QString path)
{
    ...
    if (totalZipSize >= storage.bytesAvailable()) {
        if (!MScore::noGui) {
            QMessageBox::critical(mscore, QWidget::tr("Import Extension File"), QWidget::tr(
                                      "Cannot import extension: storage %1 is full").arg(storage.displayName()));
        }
        return false;
    }
    ...
    QSet<QString> acceptableFolders
        = { Extension::sfzsDir, Extension::soundfontsDir, Extension::templatesDir, Extension::instrumentsDir, Extension::workspacesDir };
    for (auto fi : zipFile.fileInfoList()) {
    ...
    if (dirList.contains(extensionId)) {
        ...
        if (!newerVersion) {
            ...
            if (!d.removeRecursively()) {
                if (!MScore::noGui) {
                    QMessageBox::critical(mscore, QWidget::tr("Import Extension File"),
                                          QWidget::tr("Error while deleting previous version of the extension: %1").arg(extensionId));
                }
                return false;
            }
        }
    ...
    auto loadSoundFontAsync = [&]() {
        ...
        QDir sfzDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(version).arg(
                        Extension::sfzsDir));
        if (sfzDir.exists()) {
            // get all sfz files
        ...
        QDir sfDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(version).arg(
                       Extension::soundfontsDir));
        if (sfDir.exists()) {
    ...
    QDir workspacesDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(version).arg(
...
bool MuseScore::uninstallExtension(QString extensionId)
{
    ...
    QDir sfzDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(version).arg(Extension::
                                                                                                                                 sfzsDir));
    if (sfzDir.exists()) {
        // get all sfz files
        QDirIterator it(sfzDir.absolutePath(), QStringList("*.sfz"), QDir::Files, QDirIterator::Subdirectories);
    ...
    QDir sfDir(QString("%1/%2/%3/%4").arg(preferences.getString(PREF_APP_PATHS_MYEXTENSIONS)).arg(extensionId).arg(version).arg(Extension::
                                                                                                                                soundfontsDir));
    if (sfDir.exists()) {
        // get all soundfont files
        QStringList filters("*.sf2");
...
PaletteWorkspace* MuseScore::getPaletteWorkspace()
{
    if (!paletteWorkspace) {
        ...
        if (WorkspacesManager::currentWorkspace()) {
            connect(paletteWorkspace, &PaletteWorkspace::userPaletteChanged, WorkspacesManager::currentWorkspace(),
                    QOverload<>::of(&Workspace::setDirty), Qt::UniqueConnection);
        }
    }
        ...
        if (!name.isEmpty()) {
            ...
            if (!cs->loadStyle(name)) {
                QMessageBox::StandardButton b = QMessageBox::warning(this, tr("Load Style"),
                                                                     tr("MuseScore may not be able to load this style file: %1").arg(MScore
                                                                                                                                     ::
                                                                                                                                     lastError),
...
void MuseScore::updateUiStyleAndTheme()
{
    ...
    QString wd
        = QDir::cleanPath(QString("%1/../../../Data/%2").arg(QCoreApplication::applicationDirPath()).arg(
    ...
    if (preferences.isThemeDark()) {
        css.replace("$buttonHighlightDisabledOff", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_DARK_OFF).name().toLatin1());
        css.replace("$buttonHighlightDisabledOn", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_DARK_ON).name().toLatin1());
        css.replace("$buttonHighlightEnabledOff", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_DARK_OFF).name().toLatin1());
        css.replace("$buttonHighlightEnabledOn",
                    Ms::preferences.getColor(PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_DARK_ON).name().toLatin1());
    } else {
        css.replace("$buttonHighlightDisabledOff", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_LIGHT_OFF).name().toLatin1());
        css.replace("$buttonHighlightDisabledOn", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_DISABLED_LIGHT_ON).name().toLatin1());
        css.replace("$buttonHighlightEnabledOff", Ms::preferences.getColor(
                        PREF_UI_BUTTON_HIGHLIGHT_COLOR_ENABLED_LIGHT_OFF).name().toLatin1());
        css.replace("$buttonHighlightEnabledOn",
...
MuseScoreApplication::CommandLineParseResult MuseScoreApplication::parseCommandLineArguments(MuseScoreApplication* app)
{
    ...
    parser.addOption(QCommandLineOption({ "r", "image-resolution" }, "Use with '-o <file>.png'. Set output resolution for image export",
                                        "DPI"));
    ...
    parser.addOption(QCommandLineOption({ "E", "install-extension" },
                                        "Install an extension, load soundfont as default unless -e is passed too", "extension file"));
...
void MuseScore::init(QStringList& argv)
{
    ...
    if (dataPath.isEmpty()) {
        dataPath
            = QDir::cleanPath(QString("%1/../../../Data/settings").arg(QCoreApplication::applicationDirPath()).arg(
                                  QCoreApplication::applicationName()));
...
//                  qDebug("set paper size from default printer");
    ...
    if (MScore::debugMode) {
        ...
        qDebug() << "  Available virtual geometry:" << screen->availableVirtualGeometry().x() << screen->availableVirtualGeometry().y() <<
            screen->availableVirtualGeometry().width() << "x" << screen->availableVirtualGeometry().height();
        qDebug() << "  Available virtual size:" << screen->availableVirtualSize().width() << "x" << screen->availableVirtualSize().height();
        qDebug() << "  Depth:" << screen->depth() << "bits";
        qDebug() << "  Geometry:" << screen->geometry().x() << screen->geometry().y() << screen->geometry().width() << "x" <<
