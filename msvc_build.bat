...
IF NOT "%QT_MAJOR_VERSION%"=="5" IF NOT "%QT_MAJOR_VERSION%"=="6" (
   ECHO Invalid QT_MAJOR_VERSION: %QT_MAJOR_VERSION%. Use 5 or 6.
   EXIT /B 2
)
...
:FIND_GENERATOR
   ...
   REM Fall back to Visual Studio 2019 and then 2017.
...
REM -DCMAKE_BUILD_NUMBER=%BUILD_NUMBER% -DCMAKE_BUILD_AUTOUPDATE=%BUILD_AUTOUPDATE% %CRASH_REPORT_URL_OPT% are used for CI only
   ...
   IF EXIST "CMakeCache.txt" (
      ...
      cmake -G "%GENERATOR_NAME%" -A "%PLATFORM_NAME%" -DCMAKE_INSTALL_PREFIX=../%INSTALL_FOLDER% -DCMAKE_BUILD_TYPE=%CONFIGURATION_STR% -DMUSESCORE_BUILD_CONFIG=%MUSESCORE_BUILD_CONFIG% -DMUSESCORE_REVISION=%MUSESCORE_REVISION% -DMSCORE_QT_MAJOR_VERSION=%QT_MAJOR_VERSION% -DBUILD_FOR_WINSTORE=%BUILD_FOR_WINSTORE% -DBUILD_64=%BUILD_64% -DCMAKE_BUILD_NUMBER=%BUILD_NUMBER% -DBUILD_AUTOUPDATE=%BUILD_AUTOUPDATE% %CRASH_REPORT_URL_OPT% %TELEMETRY_TRACK_ID_OPT% %WIN_PORTABLE_OPT% ..
      IF !ERRORLEVEL! NEQ 0 (
