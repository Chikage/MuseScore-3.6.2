...
void Fluid::play(const PlayEvent& event)
      ...
      if (type == ME_NOTEON) {
            ...
            if (vel == 0) {
                  ...
                  // process note off
                  //
                  for (Voice* v : std::as_const(activeVoices)) {
...
void Fluid::system_reset()
      ...
      for(Voice* v : std::as_const(activeVoices))
            v->off();
      for(Channel* c : std::as_const(channel))
            c->reset();
      }
...
Preset* Fluid::find_preset(unsigned banknum, unsigned prognum)
      {
      for (SFont* sf : std::as_const(sfonts)) {
            Preset* preset = sf->get_preset(banknum, prognum);
            if (preset)
                  return preset;
            }
...
void Fluid::start_voice(Voice* voice)
      ...
      */
      ...
      if (excl_class) {
            ...
            for(Voice* existing_voice : std::as_const(activeVoices)) {
...
void Fluid::set_gen2(int chan, int param, float value, int absolute, int normalized)
      ...
      for(Voice* vo : std::as_const(activeVoices)) {
            if (vo->chan == chan)
...
SynthesizerGroup Fluid::state() const
      ...
      for (const QString &sf : std::as_const(sfl))
