...
SFont::~SFont()
      {
      for(Sample* s : std::as_const(sample))
            delete s;
      for(Preset* p : std::as_const(presets))
            delete p;
      for(unsigned char* p : std::as_const(infos))
            delete[] p;
      for(Instrument* i : std::as_const(instruments)) {
//            for(Zone* z : i->zones)
//                  delete z;
            delete i;
            }
...
bool SFont::read(const QString& s)
      ...
      for (auto instrument : std::as_const(instruments)) {
      ...
      for (auto preset : std::as_const(presets)) {
            if (synth->loadWasCanceled())
                  return false;
...
void Preset::loadSamples()
      ...
      if (_global_zone && _global_zone->instrument) {
            ...
            for (Zone* iz : std::as_const(i->zones))
                  iz->sample->load();
            }
...
bool Preset::noteon(Fluid* synth, unsigned id, int chan, int key, int vel, double nt)
      ...
      for (Zone* preset_zone : std::as_const(zones)) {
            ...
            if (preset_zone->inside_range(key, vel)) {
                  ...
                  for(Zone* inst_zone : inst->get_zone()) {
                        ...
                        if (inst_zone->inside_range(key, vel) && (sample != 0)) {
                              ...
                              if (global_inst_zone){
                                    for(Mod* mod1 : std::as_const(global_inst_zone->modlist))
                               ...
                               */
                              for(Mod* mod1 : std::as_const(inst_zone->modlist)) {
                              ...
                              if (global_preset_zone){
                                    for(Mod* mod1 : std::as_const(global_preset_zone->modlist))
                                          mod_list[mod_list_count++] = mod1;
                                    }
                              ...
                              for(Mod* mod1 : std::as_const(preset_zone->modlist)) {
                                    for (int i = 0; i < mod_list_count; i++) {
                                          if (mod_list[i] && test_identity(mod1,mod_list[i]))
                                                mod_list[i] = 0;
                                          }
...
bool Preset::importSfont()
      ...
      int idx = 0;
      for(Zone* zone : std::as_const(zones)) {
...
Zone::~Zone()
      {
      for(Mod* m : std::as_const(modlist))
            delete m;
      for(SFGen* p : std::as_const(gen))
            delete p;
      for(SFMod* p : std::as_const(mod))
            delete p;
      }
...
bool Zone::importZone()
      {
      for (SFGen* sfgen : std::as_const(gen)) {
            switch (sfgen->id) {
                  case GEN_KEYRANGE:
      ...
      for(SFMod* mod_src : std::as_const(mod)) {
...
void SFont::load_pmod (int size)
      {
      for (Preset* p : std::as_const(presets)) {
            for(Zone* p2 : std::as_const(p->zones)) {
...
void SFont::load_ibag(int size)
      ...
      for(Instrument* in : std::as_const(instruments)) {
            int n = in->zones.size();
            for (int i = 0; i < n; ++i) {
	            if ((size -= SFBAGSIZE) < 0) {
...
void SFont::load_imod(int size)
      {
      for(Instrument* i : std::as_const(instruments)) {
            for(Zone* p2 : std::as_const(i->zones)) {
...
void SFont::load_igen (int size)
      {
      for(Instrument* instr : std::as_const(instruments)) {
            bool gzone     = false;
            bool discarded = false;
...
void SFont::fixup_pgen()
      {
      for(Preset* p : std::as_const(presets)) {
            for(Zone* z : std::as_const(p->zones)) {
...
void SFont::fixup_igen()
      {
      for(Instrument* p : std::as_const(instruments)) {
            for(Zone* z : std::as_const(p->zones)) {
                  if (z->sampIdx) {
