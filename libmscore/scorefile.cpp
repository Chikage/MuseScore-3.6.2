...
static constexpr int MSC_COMPAT_READ_VERSION = 470;

//---------------------------------------------------------
//   msczAudioVolumeToMidi
//---------------------------------------------------------

static int msczAudioVolumeToMidi(double volumeDb)
      {
      if (!qIsFinite(volumeDb))
            return Channel::defaultVolume;
      if (volumeDb <= -60.0)
            return 0;
      return qBound(0, qRound(Channel::defaultVolume * qPow(10.0, volumeDb / 40.0)), 127);
      }

//---------------------------------------------------------
//   msczAudioBalanceToPan
//---------------------------------------------------------

static int msczAudioBalanceToPan(double balance)
      {
      if (!qIsFinite(balance))
            return 64;
      double normalized = qAbs(balance) <= 1.0 ? balance : balance / 100.0;
      normalized = qBound(-1.0, normalized, 1.0);
      const double pan = normalized < 0.0 ? 64.0 + normalized * 64.0 : 64.0 + normalized * 63.0;
      return qBound(0, qRound(pan), 127);
      }
...
static int msczAudioSendToMidi(double amount)
      {
      if (!qIsFinite(amount))
            return -1;
      double normalized = qAbs(amount) <= 1.0 ? amount : amount / 100.0;
      return qBound(0, qRound(qBound(0.0, normalized, 1.0) * 127.0), 127);
      }

//---------------------------------------------------------
//   msczAudioAuxEffectName
//---------------------------------------------------------

static QString msczAudioAuxEffectName(const QJsonObject& aux)
      {
      const QJsonObject fxChain = aux.value("out").toObject().value("fxChain").toObject();
      for (auto it = fxChain.constBegin(); it != fxChain.constEnd(); ++it) {
            const QJsonObject resourceMeta = it.value().toObject().value("resourceMeta").toObject();
            const QString id = resourceMeta.value("id").toString();
            if (!id.isEmpty())
                  return id;
            }
      return QString();
...
//---------------------------------------------------------
//   msczAudioAuxRoleIndex
//---------------------------------------------------------

static int msczAudioAuxRoleIndex(const QJsonArray& aux, const QString& role)
      {
      const QString lowerRole = role.toLower();
      for (int i = 0; i < aux.size(); ++i) {
            const QString effectName = msczAudioAuxEffectName(aux.at(i).toObject()).toLower();
            if (effectName.contains(lowerRole))
                  return i;
            }
      return -1;
      }
...
//---------------------------------------------------------
//   msczAudioAuxSendAmount
//---------------------------------------------------------

static int msczAudioAuxSendAmount(const QJsonValue& auxSendsValue, int index)
      {
      if (index < 0)
            return -1;

      QJsonObject send;
      if (auxSendsValue.isArray()) {
            const QJsonArray sends = auxSendsValue.toArray();
            if (index >= sends.size())
                  return -1;
            send = sends.at(index).toObject();
            }
      else if (auxSendsValue.isObject()) {
            send = auxSendsValue.toObject().value(QString::number(index)).toObject();
            }

      if (send.isEmpty())
            return -1;
      if (send.contains("active") && !send.value("active").toBool())
            return -1;
      return msczAudioSendToMidi(send.value("signalAmount").toDouble(-1.0));
      }

//---------------------------------------------------------
//   setMsczAudioMasterGain
//---------------------------------------------------------

static void setMsczAudioMasterGain(MasterScore* score, const QJsonObject& master)
      {
      if (!master.contains("volumeDb"))
            return;

      const double volumeDb = master.value("volumeDb").toDouble(0.0);
      if (!qIsFinite(volumeDb))
            return;

      const double gain = qBound(0.0, 0.1 * qPow(10.0, volumeDb / 20.0), 10.0);
      ...
      for (SynthesizerGroup& group : state) {
            if (group.name() != "master")
                  continue;
            foundMaster = true;
            for (IdValue& value : group) {
                  if (value.id == 2) {
                        value.data = QString::number(gain);
                        foundGain = true;
                        break;
                        }
                  }
            if (!foundGain)
                  group.push_back(IdValue(2, QString::number(gain)));
            break;
            }
      if (!foundMaster) {
...
static Part* msczAudioPart(MasterScore* score, const QJsonObject& track, int fallbackPartIndex)
      {
      bool ok = false;
      int partIndex = -1;
      const QJsonValue partId = track.value("partId");
      if (partId.isString())
            partIndex = partId.toString().toInt(&ok) - 1;
      else if (partId.isDouble()) {
            partIndex = partId.toInt() - 1;
            ok = true;
            }

      if (ok) {
            if (partIndex >= 0 && partIndex < score->parts().size())
                  return score->parts().at(partIndex);
            return nullptr;
            }

      const QString partIdString = partId.toString();
      if (!partIdString.isEmpty()) {
            for (Part* part : score->parts()) {
                  if (part->id() == partIdString)
                        return part;
                  }
            }

      if (fallbackPartIndex >= 0 && fallbackPartIndex < score->parts().size())
            return score->parts().at(fallbackPartIndex);
      return nullptr;
      }
...
static Channel* msczAudioChannel(Part* part, const QString& instrumentId)
      {
      if (!part || !part->instrument())
            return nullptr;

      Instrument* instrument = part->instrument();
      if (instrument->channel().isEmpty())
            return nullptr;
      if (instrumentId == "chord_symbols") {
            part->updateHarmonyChannels(false);
            const int channelIndex = instrument->channelIdx(Channel::HARMONY_NAME);
            return channelIndex >= 0 ? instrument->channel(channelIndex) : nullptr;
            }

      return instrument->channel(0);
      }
...
static void applyMsczAudioTrack(Channel* channel, const QJsonObject& track, int reverbAuxIndex, int chorusAuxIndex)
      {
      if (!channel)
            return;

      const QJsonObject out = track.value("out").toObject();
      if (out.contains("volumeDb"))
            channel->setVolume(char(msczAudioVolumeToMidi(out.value("volumeDb").toDouble(0.0))));
      if (out.contains("balance"))
            channel->setPan(char(msczAudioBalanceToPan(out.value("balance").toDouble(0.0))));

      const QJsonValue auxSends = out.value("auxSends");
      const int reverb = msczAudioAuxSendAmount(auxSends, reverbAuxIndex);
      if (reverb >= 0)
            channel->setReverb(char(reverb));
      const int chorus = msczAudioAuxSendAmount(auxSends, chorusAuxIndex);
      if (chorus >= 0)
            channel->setChorus(char(chorus));

      const QJsonObject soloMuteState = track.value("soloMuteState").toObject();
      if (soloMuteState.contains("mute"))
            channel->setMute(soloMuteState.value("mute").toBool());
      if (soloMuteState.contains("solo"))
            channel->setSolo(soloMuteState.value("solo").toBool());
      }

//---------------------------------------------------------
//   loadMsczAudioSettings
//---------------------------------------------------------

static void loadMsczAudioSettings(MasterScore* score, const QByteArray& data)
      {
      if (data.isEmpty())
            return;
      ...
      setMsczAudioMasterGain(score, root.value("master").toObject());

      const QJsonArray aux = root.value("aux").toArray();
      int reverbAuxIndex = msczAudioAuxRoleIndex(aux, "reverb");
      if (reverbAuxIndex < 0 && !aux.isEmpty())
            reverbAuxIndex = 0;
      const int chorusAuxIndex = msczAudioAuxRoleIndex(aux, "chorus");

      const QJsonValue tracksValue = root.value("tracks");
      int fallbackPartIndex = 0;
      auto applyTrack = [&](const QJsonObject& track) {
            const QString instrumentId = track.value("instrumentId").toString();
            if (instrumentId == "metronome" || track.value("partId").toString() == "999")
                  return;

            Part* part = msczAudioPart(score, track, instrumentId == "chord_symbols" ? -1 : fallbackPartIndex);
            if (instrumentId != "chord_symbols" && part)
                  ++fallbackPartIndex;
            applyMsczAudioTrack(msczAudioChannel(part, instrumentId), track, reverbAuxIndex, chorusAuxIndex);
            };

      if (tracksValue.isArray()) {
            const QJsonArray tracks = tracksValue.toArray();
            for (const QJsonValue& value : tracks)
                  applyTrack(value.toObject());
            }
      else if (tracksValue.isObject()) {
            const QJsonObject tracks = tracksValue.toObject();
            for (auto it = tracks.constBegin(); it != tracks.constEnd(); ++it)
                  applyTrack(it.value().toObject());
            }
      }
...
static bool isMscxFile(const QString& path)
      {
      return path.endsWith(".mscx", Qt::CaseInsensitive);
      }

static bool isRootLevelFile(const QString& path)
      {
      return !path.contains('/');
      }

static bool isPictureFile(const QString& path)
      {
      return path.startsWith("Pictures/", Qt::CaseInsensitive);
      }

static void appendUnique(QList<QString>& list, const QString& value)
      {
      if (!value.isEmpty() && !list.contains(value))
            list.append(value);
      }

static QString chooseRootFile(const QList<QString>& rootFiles)
      {
      for (const QString& path : rootFiles) {
            if (isRootLevelFile(path) && isMscxFile(path))
                  return path;
            }
      for (const QString& path : rootFiles) {
            if (isMscxFile(path))
      ...
      return rootFiles.isEmpty() ? QString() : rootFiles.front();
      }
            ...
            while (e.readNextStartElement()) {
                  ...
                  while (e.readNextStartElement()) {
                        ...
                        if (tag == "rootfile") {
                              ...
                              appendUnique(rootFiles, path);
                              if (isPictureFile(path))
                                    appendUnique(images, path);
                              e.skipCurrentElement();
                              }
                        else if (tag == "file")
                              appendUnique(images, e.readElementText());
                        else
                              e.unknown();
                        }
                  }
            }
      return chooseRootFile(rootFiles);
      }

static void loadMsczStyleFile(MasterScore* score, const QByteArray& styleData)
      {
      if (styleData.isEmpty())
            return;

      XmlReader e(styleData);
      while (e.readNextStartElement()) {
            if (e.name() != "museScore") {
                  e.unknown();
                  continue;
...
static void loadMsczChordListFile(MasterScore* score, const QByteArray& chordListData)
      {
      if (chordListData.isEmpty())
            return;

      XmlReader e(chordListData);
      while (e.readNextStartElement()) {
            if (e.name() != "museScore") {
                  e.unknown();
                  continue;
...
Score::FileError MasterScore::loadCompressedMsc(QIODevice* io, bool ignoreVersionError)
      ...
      const QByteArray styleData = uz.fileData("score_style.mss");
      const QByteArray chordListData = uz.fileData("chordlist.xml");
      const QByteArray audioSettingsData = uz.fileData("audiosettings.json");
      ...
      if (retval == FileError::FILE_NO_ERROR && !styleData.isEmpty()) {
            loadMsczStyleFile(this, styleData);
            styleChanged();
            }
      if (retval == FileError::FILE_NO_ERROR && !chordListData.isEmpty()) {
            loadMsczChordListFile(this, chordListData);
            styleChanged();
            }
      if (retval == FileError::FILE_NO_ERROR && !audioSettingsData.isEmpty())
            loadMsczAudioSettings(this, audioSettingsData);
      ...
      while (e.readNextStartElement()) {
            if (e.name() == "museScore") {
                  ...
                  if (!ignoreVersionError) {
                        if (mscVersion() > MSC_COMPAT_READ_VERSION)
