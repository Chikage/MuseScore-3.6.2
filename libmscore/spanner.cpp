...
//---------------------------------------------------------
//   findCRinVisualStaff
//    find last chord/rest visually placed on staff that ends before tick
//---------------------------------------------------------

static ChordRest* findCRinVisualStaff(const Score* score, const Fraction& tick, int staffIdx)
      {
      Fraction ptick = tick - Fraction::fromTicks(1);
      Measure* m = score->tick2measureMM(ptick);
      if (!m)
            return nullptr;
      if (m->isMMRest())
            ptick = m->tick();

      ChordRest* result = nullptr;
      Fraction lastTick = Fraction(-1, 1);
      for (Segment* ns = m->first(SegmentType::ChordRest); ns; ns = ns->next(SegmentType::ChordRest)) {
            if (ns->tick() > ptick)
                  break;
            for (int track = 0; track < score->ntracks(); ++track) {
                  ChordRest* cr = toChordRest(ns->element(track));
                  if (!cr || cr->vStaffIdx() != staffIdx)
                        continue;
                  Fraction endTick = cr->tick() + cr->actualTicks();
                  if (endTick > tick || endTick < lastTick)
                        continue;
                  if (endTick > lastTick || !result || (!result->isChord() && cr->isChord())) {
                        result = cr;
                        lastTick = endTick;
                        }
                  }
      ...
      switch (_anchor) {
            case Anchor::SEGMENT: {
                  ...
                  if (isLyricsLine() && toLyricsLine(this)->isEndMelisma()) {
                        ...
                        if (!_endElement && isOttava())
                              _endElement = findCRinVisualStaff(score(), tick2(), track2() / VOICES);
                        }
                  if (!_endElement) {
