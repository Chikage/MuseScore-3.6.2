...
static QString compatibleConnectorName(const QString& name)
      {
      if (name == "GuitarBend")
            return "Glissando";
      return name;
...
static bool isUnsupportedMuseScore4Connector(const QString& name)
      {
      return name == "PartialTie"
          || name == "GuitarBendHold"
          || name == "LaissezVib"
          || name == "LaissezVibSegment"
          || name == "PartialTieSegment"
          || name == "GuitarBendSegment";
      }
...
bool ConnectorInfoReader::read()
      ...
      if (_type == ElementType::INVALID) {
            if (!isUnsupportedMuseScore4Connector(rawName))
                  qDebug("ConnectorInfoReader::read: unsupported connector type <%s>", qPrintable(rawName));
            return false;
            }
