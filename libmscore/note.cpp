...
Element* Note::drop(EditData& data)
      ...
      switch(e->type()) {
            ...
            case ElementType::GLISSANDO:
                  {
                  for (auto ee : std::as_const(_spannerFor)) {
...
void Note::setDotY(Direction pos)
      ...
      for (NoteDot* dot : std::as_const(_dots)) {
            dot->layout();
            dot->rypos() = y;
            }
      }
...
void Note::scanElements(void* data, void (*func)(void*, Element*), bool all)
      ...
      for (Spanner* sp : std::as_const(_spannerFor))
...
void Note::setTrack(int val)
      ...
      for (Spanner* s : std::as_const(_spannerFor)) {
            s->setTrack(val);
            }
      for (Spanner* s : std::as_const(_spannerBack)) {
            s->setTrack2(val);
            }
      for (Element* e : _el)
            e->setTrack(val);
...
void Note::endDrag(EditData& ed)
      ...
      for (Note* nn : tiedNotes()) {
            for (const PropertyData& pd : std::as_const(ned->propertyData)) {
      ...
      switch (e->type()) {
            ...
            case ElementType::BEND: {
                  ...
                  if (next)
                        ...
                        for (auto i : std::as_const(_spannerFor)) {
