...
static bool isOttavaAlta(OttavaType t)
      {
      return t == OttavaType::OTTAVA_8VA || t == OttavaType::OTTAVA_15MA || t == OttavaType::OTTAVA_22MA;
      }
...
void Ottava::read(XmlReader& e)
      ...
      while (e.readNextStartElement())
            readProperties(e);
      if (score()->mscVersion() >= 400 && isOttavaAlta(_ottavaType) && placeBelow())
            setPlacement(Placement::ABOVE);
      if (score()->mscVersion() >= 400 && placeBelow()) {
            if (!isStyled(Pid::BEGIN_HOOK_HEIGHT)) {
                  Spatium beginHookHeight = getProperty(Pid::BEGIN_HOOK_HEIGHT).value<Spatium>();
                  setProperty(Pid::BEGIN_HOOK_HEIGHT, -beginHookHeight);
                  }
            if (!isStyled(Pid::END_HOOK_HEIGHT)) {
                  Spatium endHookHeight = getProperty(Pid::END_HOOK_HEIGHT).value<Spatium>();
                  setProperty(Pid::END_HOOK_HEIGHT, -endHookHeight);
                  }
            }
