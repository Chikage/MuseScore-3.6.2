      ...
      for (MasterScore* ms : *movements()) {
            ...
            if (updateAll || cs.updateAll()) {
                  for (Score* s : scoreList()) {
                        for (MuseScoreView* v : std::as_const(s->viewer)) {
                              v->updateAll();
                              }
                        }
                  }
...
void Score::deletePostponed()
      {
      for (ScoreElement* e : std::as_const(_updateState._deleteList)) {
            if (e->isSystem()) {
                  System* s = toSystem(e);
...
void Score::changeCRlen(ChordRest* cr, const Fraction& dstF, bool fillWithRest)
      ...
      bool first = true;
      for (Fraction f2 : std::as_const(flist)) {
            f  -= f2;
...
void Score::upDown(bool up, UpDownMode mode)
      ...
      for (Note* oNote : std::as_const(el)) {
            ...
            if ((oNote->pitch() != newPitch) || (oNote->tpc1() != newTpc1) || oNote->tpc2() != newTpc2) {
                  ...
                  if (mode != UpDownMode::OCTAVE) {
                        ...
                        for (ScoreElement* e : std::as_const(l)) {
                              Note* ln = toNote(e);
                              if (ln->accidental())
      ...
      if (el) {
            ...
            if (noteEntryMode()) {
                  ...
                  if (_is.cr() || !el->selected())
                        ...
                        setPlayNote(false);
                  for (MuseScoreView* view : std::as_const(viewer))
                        view->moveCursor();
                  }
            else {
                  select(el, SelectType::SINGLE, 0);
