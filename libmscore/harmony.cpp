...
void Harmony::read(XmlReader& e)
      ...
      while (e.readNextStartElement()) {
            ...
            if (tag == "base" || tag == "bass")
                  ...
                  if (score()->mscVersion() >= 400 && _rootTpc == Tpc::TPC_INVALID) {
                        const Staff* st = staff();
                        const Key key = st ? st->key(tick()) : Key::C;
                        const int rootTpc = function2Tpc(_function, key);
                  ...
                  while (e.readNextStartElement()) {
                        ...
                        else if (harmonyInfoTag == "extension") {
                              const int extension = e.readInt();
                              if (applyHarmonyInfo)
                                    setId(extension);
                              }
                        else if (harmonyInfoTag == "name") {
                              const QString name = e.readElementText();
                              if (applyHarmonyInfo)
                                    _textName = name;
                              }
                        else if (harmonyInfoTag == "root") {
                              const int rootTpc = e.readInt();
                              if (applyHarmonyInfo)
                                    setRootTpc(rootTpc);
                              }
                        else
