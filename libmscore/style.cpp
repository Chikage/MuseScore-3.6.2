...
static int lineStyleFromString(const QString& value)
      {
      bool ok = false;
      ...
      if (s == "dashed" || s == "dash")
            return int(Qt::DashLine);
      if (s == "dotted" || s == "dot")
            return int(Qt::DotLine);
      if (s == "dashdot" || s == "dash-dotted" || s == "dashdotted")
            return int(Qt::DashDotLine);
      if (s == "dashdotdot" || s == "dash-dot-dotted" || s == "dashdotdotted")
            return int(Qt::DashDotDotLine);
      if (s == "custom" || s == "customdash" || s == "custom-dash")
            return int(Qt::CustomDashLine);
      if (s == "none" || s == "nopen" || s == "no-pen")
            return int(Qt::NoPen);
      return int(Qt::SolidLine);
      }
...
static bool isLineStyleSid(Sid idx)
      {
      switch (idx) {
            case Sid::hairpinLineStyle:
            case Sid::hairpinLineLineStyle:
            case Sid::pedalLineStyle:
            case Sid::voltaLineStyle:
            case Sid::ottavaLineStyle:
            case Sid::letRingLineStyle:
            case Sid::palmMuteLineStyle:
                  return true;
            default:
                  return false;
            }
      ...
      for (const StyleType& t : styleTypes) {
            ...
            if (t.name() == tag) {
                  ...
                  if (!strcmp("Ms::Spatium", type))
                        ...
                        const QString value = e.readElementText();
                        set(idx, QVariant(isLineStyleSid(idx) ? lineStyleFromString(value) : value.toInt()));
                        }
...
void MStyle::load(XmlReader& e)
      ...
      while (e.readNextStartElement()) {
            ...
            else if (tag == "pedalLineStyle")
                  set(Sid::pedalLineStyle, lineStyleFromString(e.readElementText()));
...
void MStyle::applyNewDefaults(const MStyle& other, const int defaultsVersion)
      ...
      for (auto st : std::as_const(styleTypes))
