...
//                                    x = -cr->spaceLw;  // account for accidentals, etc
                        ...
                        if (isOttava()) {
                              if (cr && cr->durationType() == TDuration::DurationType::V_MEASURE) {
                                    ...
                                    const int lineStaffIdx = staffIdx();
                                    int startTrack = lineStaffIdx * VOICES;
                                    int endTrack   = startTrack + VOICES;
                                    qreal width    = cr->rightEdge();
                                    ...
                                    for (int track = 0; track < score()->ntracks(); ++track) {
                                          ...
                                          if (!cr1)
                                                continue;
                                          const bool onLineStaff = track >= startTrack && track < endTrack;
                                          if (!onLineStaff && cr1->vStaffIdx() != lineStaffIdx)
                                                continue;
                                          if (cr1->isChord()) {
                                    ...
                                    while (ns) {
                                          for (int track = 0; track < score()->ntracks(); ++track) {
                                                ChordRest* cr1 = toChordRest(ns->element(track));
                                                if (!cr1)
                                                      continue;
                                                const bool onLineStaff = track >= startTrack && track < endTrack;
                                                if (onLineStaff || cr1->vStaffIdx() == lineStaffIdx) {
...
bool SLine::readProperties(XmlReader& e)
      ...
      if (tag == "tick2") {                // obsolete
            ...
            _lineStyle = Qt::PenStyle(propertyFromString(Pid::LINE_STYLE, e.readElementText()).toInt());
