...
//      void spanners13();            // drop a line break at the middle of a LyricsLine and check LyricsLineSegments
      ...
      void spanners17();            // read an ottava ending on a cross-staff chord
      };
...
//---------------------------------------------------------
///  spanners17
///   read an ottava ending on a cross-staff chord
//---------------------------------------------------------

void TestSpanners::spanners17()
      {
      MasterScore* score = readScore(DIR + "ottava-crossstaff.mscx");
      QVERIFY(score);
      ...
      for (auto it = score->spanner().cbegin(); it != score->spanner().cend(); ++it) {
            Spanner* spanner = (*it).second;
            if (spanner->isOttava()) {
                  ottava = toOttava(spanner);
                  break;
                  }
            }

      QVERIFY(ottava);
      QCOMPARE(ottava->staffIdx(), 1);
      QVERIFY(ottava->endElement());

      ChordRest* endCr = toChordRest(ottava->endElement());
      QVERIFY(endCr);
      QCOMPARE(endCr->vStaffIdx(), 1);

      QVERIFY(ottava->lineVisible());
      QVERIFY(ottava->lineWidth() > 0.0);
      QVERIFY(ottava->nsegments() > 0);
      QVERIFY(ottava->frontSegment()->pos2().x() > score->spatium() * 2);
